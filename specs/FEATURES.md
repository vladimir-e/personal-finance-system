...
The add-transaction modal opens with a hero amount input (large, type-aware coloring) and a semantic segmented control for type (color-coded: red expense, green income, blue transfer). A flexible date input accepts shorthand formats (`2/15`, `15`, `yesterday`) with a calendar popup for touch selection. Secondary fields (description, payee, notes) are collapsed behind a "Details" toggle for fast entry. Keyboard shortcuts: `Cmd/Ctrl+1/2/3` switches type, `+`/`-` in the amount field switches type, `Cmd/Ctrl+Enter` submits from any field. Transfer creation simultaneously generates the outflow and inflow legs. Deleting one leg cascades to delete the other.
